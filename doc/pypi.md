...
PyPI（Python Package Index）官方原则上不支持 snapshot（快照）版本和类似 Maven、npm 的“非正式”或“临时”版本发布机制。

但你可以用如下方式间接实现：

PyPI 上发布 snapshot 的主流做法是通过版本号标识快照
在版本号后添加如 dev, alpha, beta, rc 等后缀，如：
1.2.3.dev20240101
2.0.0a1
0.9.0b2
这种命名不会影响上传，但这只是“预发布版”或“开发版”，不是真正的可变“快照版”。
PyPI 一旦上传后版本号不可重复上传，新快照必须换新版本号。不可覆盖发布

PyPI 不允许重复上传同一版本号。比如你不能连续发布两个 1.0.0.dev0。
